...
bool ErrorStateKalmanFilter::GetAngularDelta(
    ...
    linear_angle_mid = 0.5*(angular_vel_curr + angular_vel_prev);
    angular_delta = delta_t*linear_angle_mid;
...
void ErrorStateKalmanFilter::UpdateOdomEstimation(
    Eigen::Vector3d &linear_acc_mid, Eigen::Vector3d &linear_angle_mid
) {
    static Eigen::Vector3d w_b = Eigen::Vector3d::Zero();

    // get deltas:
    Eigen::Vector3d angular_delta; 
    GetAngularDelta(1, 0, angular_delta, linear_angle_mid);
